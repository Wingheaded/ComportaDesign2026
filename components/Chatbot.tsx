...
const Chatbot: React.FC<ChatbotProps> = ({ language, isChatOpen, setIsChatOpen }) => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const botResponseText = await runChat(input, newMessages);
  ...
  return (
    <>
      <div className={`fixed bottom-0 right-0 p-4 sm:p-6 transition-all duration-300 ${isChatOpen ? 'opacity-0 scale-90 invisible' : 'opacity-100 scale-100 visible'}`}>
        <button
      ...
      <div className={`fixed bottom-0 right-0 sm:bottom-6 sm:right-6 w-full h-full sm:w-[380px] sm:h-[calc(100vh-6rem)] sm:max-h-[600px] bg-white rounded-none sm:rounded-xl shadow-2xl flex flex-col transition-all duration-300 origin-bottom-right ${isChatOpen ? 'opacity-100 scale-100 visible' : 'opacity-0 scale-95 invisible'}`}>

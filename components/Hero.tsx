...
const Hero: React.FC<HeroProps> = ({ language }) => {
  ...
  return (
    <section id="home" className="sticky top-0 -z-10 relative bg-light-sand min-h-screen flex flex-col md:flex-row">
      {/* Left Content Side */}
      <div className="w-full md:w-1/2 flex items-center justify-center p-8 md:p-20 pt-32 md:pt-20 order-2 md:order-1">
        <div className="max-w-xl flex flex-col items-start text-left">
          ...
          <p className="mt-4 text-base md:text-lg text-soft-black/70 leading-relaxed max-w-md opacity-0 animate-fade-in-up" style={{ animationDelay: '0.6s' }}>
            {CONTENT.hero.description[language]}
          </p>

          <div className="mt-10 flex flex-col sm:flex-row items-start gap-4 w-full opacity-0 animate-fade-in-up" style={{ animationDelay: '0.8s' }}>
            <a href="#contact" onClick={scrollToVisit} className="w-full sm:w-auto bg-soft-black text-white px-8 py-3.5 rounded-full font-medium hover:bg-opacity-80 transition-all transform hover:scale-105 text-center shadow-lg shadow-soft-black/10 cursor-pointer">
              {CONTENT.hero.ctaPrimary[language]}
            </a>
            <a href="#programme" onClick={scrollToProgramme} className="w-full sm:w-auto text-soft-black px-8 py-3.5 rounded-full font-medium border border-soft-black/20 hover:bg-white hover:border-soft-black/40 transition-all transform hover:scale-105 text-center cursor-pointer">
      ...
      <div className="w-full md:w-1/2 h-[50vh] md:h-auto relative order-1 md:order-2">
        <div className="absolute inset-0 bg-soft-gray">
          <img
            ...
            alt="Hero image"
            className="w-full h-full object-cover opacity-0 animate-scale-in grayscale-[20%] contrast-[1.05]"
          />
